...
if [ $# -lt 1 ]; then
        ...
        echo "e.g. $0 ../../src"
        echo "Env vars MONALISACOIND and MONALISACOINCLI may be used to specify the exact binaries used"
        exit 1
fi

set -f

BITCOIND=${MONALISACOIND:-${1}/monalisacoind}
CLI=${MONALISACOINCLI:-${1}/monalisacoin-cli}
