...
// MonaLisaCoin: Normally minimum difficulty blocks can only occur in between
...
unsigned int CalculateMonaLisaCoinNextWorkRequired(const CBlockIndex* pindexLast, int64_t nFirstBlockTime, const Consensus::Params& params)
{
...
CAmount GetMonaLisaCoinBlockSubsidy(int nHeight, const Consensus::Params& consensusParams, uint256 prevHash)
{
...
int64_t GetMonaLisaCoinDustFee(const std::vector<CTxOut> &vout, CFeeRate &baseFeeRate) {
    int64_t nFee = 0;
