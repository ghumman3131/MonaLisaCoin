# MonaLisaCoin Core 1.8
===================

MonaLisaCoin Core 1.8 introduces AuxPoW from block 371,337. AuxPoW is a technology
which enables miners to submit work done while mining other coins, as work
on the MonaLisaCoin block chain. MonaLisaCoin Core 1.8 also enables payment protocol
support for MonaLisaCoin (note that MonaLisaCoin's implementation of payment protocol is
...
as soon as possible afterwards in order to be able to continue to use MonaLisaCoin.
Users remaining on the previous client **will not** be able to receive or spend
MonaLisaCoin (with the exception of with other nodes who have not upgraded).
...
use "monalisacoind" you must run it the first time with the "-reindex" command line
option.
...
AuxPoW benefits MonaLisaCoin in two ways; firstly, the effective hashrate of the coin is
...
multipools from switching between MonaLisaCoin and other Scrypt coins.
...
In comparison to the AuxPoW implementation in United Scrypt Coin, MonaLisaCoin uses a
different coinbase transaction input script format which includes block height in
compliance with BIP0034.

## Payment Protocol

MonaLisaCoin Core 1.8 adds payment protocol support, which is used to streamline payments
being made to merchants.

This protocol is defined in DIP0070-DIP0072, based on the BIP standards with the same
assigned numbers. DIP standards can be found at https://github.com/monalisacoin/dips/ .
The key differencs between the Bitcoin and MonaLisaCoin payment protocols are that
the MonaLisaCoin payment request uses a "genesis" field containing the hash of the network's
...
(typically 1 MNL, or around $0.00012 per transaction).
...
of MonaLisaCoin compared to Bitcoin. This should lead to less false positives.

A problem in parsing mangled monalisacoin: URIs under Windows has been resolved, and as
part of this work network detection for monalisacoin: URIs is now more robust.
