...
*Setup instructions for a gitian build of MonaLisaCoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the MonaLisaCoin
...
- [Building MonaLisaCoin](#building-monalisacoin)
...
Clone the git repositories for monalisacoin and gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/monalisacoin/monalisacoin
```
...
Follow the instructions in [doc/release-process.md](release-process.md) in the monalisacoin repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work offline.

Building MonaLisaCoin
----------------

To build MonaLisaCoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the monalisacoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/monalisacoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/monalisacoin/monalisacoin
...
For example:
```bash
URL=https://github.com/laanwj/monalisacoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit monalisacoin=${COMMIT} --url monalisacoin=${URL} ../monalisacoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit monalisacoin=${COMMIT} --url monalisacoin=${URL} ../monalisacoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit monalisacoin=${COMMIT} --url monalisacoin=${URL} ../monalisacoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/monalisacoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/monalisacoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/monalisacoin-osx-build.assert
```
...
[monalisacoin/gitian.sigs](https://github.com/monalisacoin/gitian.sigs/) repository, or if that's not possible create a pull request. 
